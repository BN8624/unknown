...
**프로젝트를 백지에서 새 게임으로 리셋함.** 정본 = `PRODUCT_RESET_SPEC.md`. 새 게임 **「균열기사」**(세로형 방치 RPG v0.1): `Main.tscn`/`Main.gd`(화면·전투·성장·스테이지·보스·보상·저장) + `reset/GameData.gd`(밸런스·콘텐츠) + `reset/SaveSystem.gd`(저장). 메인 씬을 `Main.tscn`으로 교체. 도형 아트(AI/PNG 없음). 골드·레벨·강화 5종·저장/불러오기·설정·Web 빌드 동작 확인. **오프라인 보상·환생(균열석)+상점(영구강화 4종)·사운드(합성 SFX 8종+BGM 루프+토글)·큰수 표기·전투력 표시. 절차적 아트(글로우·별·시차 능선·안개·잔불 파티클·비네트·셰이딩 캐릭터, `reset/gen_art.py`)·2지역(잿빛 협곡, 40층)·보스 강공격 패턴(예열→대미지+화면 플래시)·첫 실행 온보딩 추가.** 아트/사운드는 모두 절차적 자체생성 CC0(`reset/gen_art.py`·`reset/gen_audio.py`). 출시 후보 상태.
