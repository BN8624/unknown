...
**TASK 018(Issue #5) UI 재배치 — 사용자 확인 완료, 커밋 단계.** 상단 재배치(줄1 레벨·골드+특성버튼 / 줄2 EXP바 / 줄3 빚·명성·보스진행, 용병HP·공격·방어·적HP 텍스트는 HP바·강화버튼·이름표에 있어 제거). 특성 버튼이 EXP 바를 침범하던 겹침을 수정(버튼 (388,6)·크기 (136,34)·폰트 20으로 줄1 안에 수용). `--verify` ALL PASS TASK_001~014, `--shot` 6장 정상, Web 빌드 재내보내기 후 **아이폰에서 겹침 해결 확인 완료.**
⚠ **다음 세션 먼저 할 일**: 미커밋이면 TASK 018 커밋·푸시 → Issue #5 보고 → 닫기.
