...
**TASK 015(Issue #3) 기술 적용·레이아웃 확정·렌더 검증 완료.** 아트 방식 컷아웃→**SD 전신 스프라이트**. 용병·늑대 `Sprite2D`(Node2D 래퍼), 배경 2레이어. GPT 확정 레이아웃값 적용(용병144·늑대92·발660·지면615) + `--shot` 모드로 540×960 렌더 직접 캡처해 4조건(패널 비겹침·발 접촉·체력바·정상 배치) 확인. 헤드리스 ALL PASS TASK_001~014, Web 빌드 정상. 색감·미세조정·최종 미감은 TASK 016.
(이전: TASK 014 적 프로필·사운드 훅 분리 완료·아이폰 확인됨.)
...
> **TASK 016 — 아이폰 아트 승인·수정**(새 Issue 또는 #3 이어서). 색감·캐릭터 2~5px·좌우 간격·배경 미세 위치·최종 미감.
> 검증: 먼저 `--shot` 렌더 스크린샷으로 1차 확인(아래 실행), 그 다음 아이폰.
> 조정 가능한 값: `MERC_DISP_H 144`/`WOLF_DISP_H 92`(표시 높이), `CHAR_FOOT_Y 660`(발), `GROUND_TOP_Y 615`(지면 상단), 레벨업 강조 `1.18`, `BG_SKY_SCROLL 30`.
> 나머지 적 아트 TASK 017, UI 재배치 018, 사운드 019, 외형 승급 020.
...
```
GUI에서 기본 전투·레벨업 확대를 캡처해 `scratchpad/shot_01_basic.png`·`shot_02_levelup.png` 저장 후 자동 종료. 스크린샷 저장 경로는 `SHOT_DIR` 상수(환경 특정 — 다른 머신은 수정 필요). 캐릭터 구조: `Sprite2D`는 Node2D 래퍼의 자식, 표시 크기는 `get_used_rect`로 실제 그림 기준 정렬.
