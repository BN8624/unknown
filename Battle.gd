...
const MERC_UPGRADED_TEX_PATH := "res://assets/characters/mercenary_upgraded.png"  # 브루노 처치 후 외형(TASK 020)
...
# 적 SD 텍스처 경로·표시 높이 (TASK_017). 크기 순서: 고블린<늑대·방패병<용병<오우거. 보스는 BRUNO_* 별도.
const ENEMY_TEX := {
	"wolf": "res://assets/characters/wolf.png",
	"goblin": "res://assets/characters/goblin.png",
	"shield": "res://assets/characters/shield.png",
	"ogre": "res://assets/characters/ogre.png",
}
const ENEMY_DISP_H := {"wolf": 92.0, "goblin": 82.0, "shield": 120.0, "ogre": 154.0}
const BRUNO_TEX_PATH := "res://assets/characters/bruno.png"
const BRUNO_DISP_H := 170.0      # 보스는 오우거(154)보다 큰 실루엣
...
var v_task020 := false   # 외형 승급 텍스처 선택 검증(TASK_020)
...
func _ready() -> void:
	...
	if not verify_mode:
		loaded = _load_game()
		_apply_merc_appearance(false)   # 저장된 boss_defeated면 업그레이드 외형으로(연출 없이)
	_spawn_enemy()
	if verify_mode:
	...
	match shot_phase:
		...
		2:
			if shot_t >= 0.14:                              # 레벨업 확대 피크 부근
				_save_shot("02_levelup"); _shot_force("goblin"); shot_phase = 3; shot_t = 0.0
		3:
			if shot_t >= 0.4:
				_save_shot("03_goblin"); _shot_force("shield"); shot_phase = 4; shot_t = 0.0
		4:
			if shot_t >= 0.4:
				_save_shot("04_shield"); _shot_force("ogre"); shot_phase = 5; shot_t = 0.0
		5:
			if shot_t >= 0.4:
		...
		9:
			if shot_t >= 0.3:
				_save_shot("09_noupgrade")
				# 외형 승급(TASK 020): boss_defeated 후 업그레이드 용병 + 일반 적
				boss_defeated = true
				_apply_merc_appearance(false)
				boss_ui.visible = false                    # 보스전 종료 상태 모사(상단 보스 패널 숨김)
				_shot_force("goblin")
				shot_phase = 10; shot_t = 0.0
		10:
			if shot_t >= 0.4:
				_save_shot("10_upgraded"); shot_phase = 11; shot_t = 0.0
		11:
			if shot_t >= 0.3:
				get_tree().quit(0)


# --shot 전용: 특정 적을 화면 안 고정 위치에 강제 생성해 한 장씩 캡처한다(전투 흐름 무시).
func _shot_force(typ: String) -> void:
	...
	var cx := 330.0
	if typ == "goblin":
		var prof := EnemyProfiles.get_profile("goblin")
		var built: Array = []
		for i in range(3):
			built.append(_build_enemy("goblin", prof, i + 1, 3, cx + i * GOBLIN_SPACING, i == 0))
		enemy = built[0]
		goblin_queue = built.slice(1)
	elif typ == "boss":
		_spawn_boss()
		if not enemy.is_empty():
			enemy.body.position.x = cx
	else:
		enemy = _build_enemy(typ, EnemyProfiles.get_profile(typ), 0, 0, cx, true)
...
# ── 용병 ─────────────────────────────────────────────────────────
# 현재 boss_defeated와 업그레이드 에셋 존재 여부로 표시할 용병 텍스처 경로를 고른다(순수 함수, 검증 결정적).
func _choose_merc_tex(defeated: bool, upgraded_exists: bool) -> String:
	if defeated and upgraded_exists:
		return MERC_UPGRADED_TEX_PATH
	return MERC_TEX_PATH   # 미처치이거나 업그레이드 에셋 누락 시 기본 용병(안전 폴백)


func _merc_tex_path() -> String:
	return _choose_merc_tex(boss_defeated, ResourceLoader.exists(MERC_UPGRADED_TEX_PATH))


func _build_merc() -> void:
	var box := Vector2(60, 90)
	var tex := _load_tex_or_null(_merc_tex_path())
	var body: CanvasItem
	if tex != null:
...
# 현재 boss_defeated에 맞는 용병 외형을 적용한다(스프라이트 텍스처만 교체·재정렬, 노드·판정·체력바 보존).
# flash=true면 짧은 승급 연출을 함께 낸다. 도형 폴백·동일 외형이면 아무것도 하지 않는다.
func _apply_merc_appearance(flash: bool) -> void:
	if merc.is_empty():
		return
	var body = merc.get("body")
	if not (body is Node2D) or body.get_child_count() == 0 or not (body.get_child(0) is Sprite2D):
		return   # 텍스처 없는 도형 폴백 상태면 외형 교체 없음
	var tex := _load_tex_or_null(_merc_tex_path())
	if tex == null:
		return
	var sp: Sprite2D = body.get_child(0)
	if sp.texture == tex:
		return   # 이미 해당 외형이면 불필요한 연출 방지
	var box := Vector2(60, 90)
	sp.texture = tex
	var used := tex.get_image().get_used_rect()
	var s: float = MERC_DISP_H / maxf(1.0, used.size.y)
	sp.scale = Vector2(s, s)
	sp.position = Vector2(
		box.x * 0.5 - (used.position.x + used.size.x * 0.5) * s,
		box.y - (used.position.y + used.size.y) * s
...
# 외형 승급 순간의 짧은 연출(확대 후 복귀 + 반짝임 + 문구). 1초 안팎.
func _play_upgrade_flash() -> void:
	if verify_mode or merc.is_empty() or not is_instance_valid(merc.body):
		return
	var b = merc.body
	b.modulate = Color(1.8, 1.7, 1.2)
	var tw := create_tween()
	tw.tween_property(b, "scale", Vector2(1.25, 1.25), 0.18)
	tw.tween_property(b, "scale", Vector2(1.0, 1.0), 0.22)
	tw.parallel().tween_property(b, "modulate", Color.WHITE, 0.4)
	_show_notice("장비가 좋아졌다", 1.0)
...
func _spawn_boss() -> void:
	...
	# 보스도 SD 전신 스프라이트(누락 시 도형 폴백). 철퇴는 텍스처에 포함.
	var tex := _load_tex_or_null(BRUNO_TEX_PATH)
	var body: CanvasItem
	if tex != null:
		body = _make_sprite_body(tex, sx, bsize, BRUNO_DISP_H)
	else:
		var rect := ColorRect.new()
		rect.color = bcolor
		rect.size = bsize
		rect.position = Vector2(sx, by)
		rect.pivot_offset = bsize * 0.5
		body = rect
	add_child(body)

	var head_y: float = (CHAR_FOOT_Y - BRUNO_DISP_H) if tex != null else by
	var hp_bg := ColorRect.new()
	hp_bg.color = Color(0, 0, 0, 0.7)
	hp_bg.size = Vector2(bar_w + 6, 14)       # 일반 적보다 넓은 보스 체력 바
	hp_bg.position = Vector2(sx - 3, head_y - 26)
	add_child(hp_bg)

	var hp_fill := ColorRect.new()
	hp_fill.color = Color(0.95, 0.25, 0.30)
	hp_fill.size = Vector2(bar_w, 12)
	hp_fill.position = Vector2(sx, head_y - 25)
	add_child(hp_fill)

	var name_label := Label.new()
	name_label.size = Vector2(240, 28)
	name_label.position = Vector2(sx + bar_w * 0.5 - 120, head_y - 56)
	...
	# 큰 철퇴 도형: 텍스처에 철퇴가 포함되므로 폴백(도형 보스) 때만 생성
	var mace: Node2D = null
	var mace_off_x := -22.0
	if tex == null:
		mace = Node2D.new()
	...
	if mace != null:   # 도형 폴백 보스만 철퇴 노드를 가진다(텍스처 보스는 그림에 포함)
		enemy["mace"] = mace
		enemy["mace_off_x"] = mace_off_x
	current_enemy_hits = 0
...
func _build_enemy(typ: String, prof: Dictionary, wave_cur: int, wave_total: int, sx: float, active: bool) -> Dictionary:
	...
	# 적 SD 전신 스프라이트(누락 시 도형 폴백). wolf/goblin/shield/ogre 매핑.
	var tex: Texture2D = _load_tex_or_null(ENEMY_TEX[typ]) if ENEMY_TEX.has(typ) else null
	var disp_h: float = float(ENEMY_DISP_H.get(typ, bsize.y))
	var body: CanvasItem
	if tex != null:
		body = _make_sprite_body(tex, sx, bsize, disp_h)
	else:
		var rect := ColorRect.new()
		rect.color = bcolor
		rect.size = bsize
	...
	var head_y: float = (CHAR_FOOT_Y - disp_h) if tex != null else by
	var hp_bg := ColorRect.new()
	hp_bg.color = Color(0, 0, 0, 0.6)
	...
	if typ == "shield" and tex == null:   # 텍스처에 방패가 포함되므로 도형 방패는 폴백 때만
		...
		e["shield"] = sh
	if typ == "ogre":
		if tex == null:                   # 몽둥이 도형도 폴백 때만(텍스처에 포함)
...
func _bruno_settlement() -> void:
	...
	_apply_merc_appearance(true)        # 외형 승급(철검+가죽) + 짧은 연출
...
func _verify_step(hits: int) -> void:
	...
	if base5 and v_traits and v_enemies and v_ogre and v_boss and v_debt and v_save and not v_task014:
		_verify_task014()
	if base5 and v_traits and v_enemies and v_ogre and v_boss and v_debt and v_save and v_task014 and not v_task020:
		_verify_task020()
	# 모든 검증 통과 시 종료 코드 0
	if base5 and v_traits and v_enemies and v_ogre and v_boss and v_debt and v_save and v_task014 and v_task020:
		print("[VERIFY] ALL PASS TASK_001~014(+020) kills=%d lv=%d atk=%d def=%d maxhp=%d (TASK_012·013은 플레이·문서로 별도 확인)" % [kill_count, level, merc.atk, merc.defense, merc.max_hp])
		get_tree().quit(0)
	if kill_count >= 120:
...
# TASK_020: boss_defeated·에셋 존재에 따른 용병 텍스처 선택(순수 함수, 에셋 유무와 무관하게 결정적).
func _verify_task020() -> void:
	if _choose_merc_tex(false, true) != MERC_TEX_PATH:
		_vfail("미처치는 기본 용병이어야 함"); return
	if _choose_merc_tex(false, false) != MERC_TEX_PATH:
		_vfail("미처치·에셋없음 기본 용병이어야 함"); return
	if _choose_merc_tex(true, true) != MERC_UPGRADED_TEX_PATH:
		_vfail("처치·에셋있음 업그레이드여야 함"); return
	if _choose_merc_tex(true, false) != MERC_TEX_PATH:
		_vfail("처치·에셋없음 기본 용병 폴백이어야 함"); return
	v_task020 = true
	print("[VERIFY] task020 외형 승급 텍스처 선택 PASS (false→기본·true+에셋→업그레이드·누락→폴백)")
